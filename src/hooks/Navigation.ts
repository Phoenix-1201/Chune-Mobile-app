...
import {useNavigationState, useNavigation, useRoute, NavigationState} from '@react-navigation/native';
import {ParamKeys} from '@/constants/Navigation';

function _routeParamSelector<T>(state: NavigationState, keyName: string): T | undefined {
...
const _screenKeySelector = (state: NavigationState, screenName: string): string | undefined => {
  const routes = state.routes || [];
  // Traverse the routes and find out the parameter
  for (let i = routes.length - 1; i >= 0; i--) {
 ...
 * @return {[type]}            [description]
 */
export function useGetNavKeyWithScreenName(screenName: string): string | undefined {
 ...
 * @return {[type]} [description]
 */
export function useNavContext<TData = any, TResult = any>(): NavContext<TData, TResult> | undefined {
