...
import ImagePicker, {Options as PickerOptions, Image} from 'react-native-image-crop-picker';
...
export default function useImagePicker(
  ...
  const [source, setSource] = React.useState<ImageSourcePropType | undefined>(initialSource);
  ...
  const pickAvatar = React.useCallback(() => {
    ...
    return new Promise((resolve, reject) => {
      showActionSheetWithOptions({options: buttonOptions, cancelButtonIndex}, async (buttonIndex) => {
        ...
        try {
          ...
          if (buttonIndex === 1) {
            ...
            reject(new ImagePickerError('Cancelled from action sheet', undefined, true));
            return;
          }
          if (isImage(picked)) {
          ...
          reject(new ImagePickerError('Unknown Error - May be cancelled from native picker or other error', undefined));
        } catch (error) {
          reject(error);
        }
      });
    });
  }, [setBase64, setSource, setContentType, showActionSheetWithOptions, pickerOptions]);
