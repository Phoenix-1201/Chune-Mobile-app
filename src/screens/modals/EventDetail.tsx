...
import { getAddressMultiline } from '@/utils/DateUtil'
// import { useStores } from '@hooks/Utils'
import { get, find, isEmpty, defaultTo } from 'lodash'
import { InviteeItem } from '@/components/general/home/OrderItem'
import OrderSummary from '@components/order/OrderSummaryYellowBox'
import { RedBtnContainer, YellowBtnContainer } from '@/components/controls/Button'
import { BlackButton } from '@components';
import { ParamKeys } from '@/constants/index'
...
const StatusInfo = ({ icon, color, name }: any) => (
    <StateView>
...
const DeliverInfo = ({ name, location, date }: any) => (
    <SectionView>
...
const HostSection = ({ onCancel, isCancellable, user, description }: any) => (
    <SectionContainer>
...
const YourOrderSection = ({ items, onEdit, onCancelOrder, event, isCancelledOrder }: any) => {
    const region = {
...
const PendingOrderSection = ({ event, onAddToOrder, onCheckout }: any) => (
    <PendingContainer>
...
const AwaitingOrderSection = ({ onAddToOrder }: any) => (
    <SectionView>
...
const InviteeOrders = ({ inviteeOrders, add, selected, props, done, event, notify }: any) => {
    return (
        <InviteeContainer>
            <InviteeOrderHeader>
                <View>
...
const EventDetail = (props: any) => {
    ...
    const addInvitees = async (users: any) => {
        // let addedUsers = [];
...
const NoteTitle = styled(AvenirNextBold)`
  font-size: 14px;
  line-height: 19px;
  color: #3f3f3f;
`

const Notes = styled(AvenirNextRegular)`
  font-size: 12px;
  line-height: 19px;
`
...
const DescTitle = styled(AvenirNextBold)`
  font-size: 13px;
  line-height: 19px;
  color: #3F3F3F;
`
...
const Rate = styled(AvenirNextBold)`
  font-size: 12px;
  line-height: 15px;
  color: #6c6c6c;
  margin-top: -2px;
...
const Status = styled(AvenirNextBold)`
  font-size: 14px;
  line-height: 19px;
  color: #3F3F3F;
  margin-bottom: 5px;
...
const AlertTitle = styled(AvenirNextBold)`
    color: #3F3F3F;
    font-size: 11px;
    line-height: 13px;
`
...
`
const SectionTitle = styled(AvenirNextBold)`
...
const DeliverSite = styled(AvenirNextBold)`
	color: #686868;
	font-size:14px;
	line-height: 17px;
`

const DeliverLocation = styled(AvenirNextRegular)`
	color: #686868;
	font-size:12px;
	line-height: 15px;
`

const DeliverTime = styled(AvenirNextBold)`
	  color: #686868;
	  font-size: 24px;
`
...
const EventButtonTitle = styled(AvenirNextBold)`
    font-size: 12px;
`
...
const NotificationText = styled(AvenirNextRegular)`
    margin-top: 10px;
    text-align: center;
    font-weight: 500;
    font-size: 11px;
...
const CartNumber = styled(AvenirNextBold)`
    font-size: 18px;
    color: black;
`
...
const StateText = styled(AvenirNextRegular)`
      font-size: 14px;
      font-weight: 600;
      color: #000000;
      margin-left: 5px;
...
const AddTitle = styled(MontBold)`
  font-size: 18px;
  color: black;
`
...
const UserName = styled(MontRegular)`
    font-size: 10px;
    line-height: 13px;
    color: #3f3f3f;
    text-align: center;
...
const EventDescription = styled(MontRegular)`
  color: #3F3F3F;
  font-size: 12px;
  margin-left: 10px;
  text-align: justify;
